<template>
    <div class="px-4">
        <div class="flex flex-wrap max-w-lg">
            <button @click="selectedTag = selectedTag !== tag ? tag : null" :class="selectedTag === tag ? 'bg-blue text-black' : 'text-blue'" class="border-2 border-blue duration-100 mb-2 mr-2 px-3 rounded-full" v-for="(tag, i) in tags" :key="`tag_${i}`">{{ tag.title[$i18n.locale] }}</button>
        </div>
