<template>
    ...
    <div class="px-4">
        <div class="flex flex-wrap mt-16 -mx-1">
            <AppLink v-for="(article, i) in filteredNews" :key="`news_${i}`" :class="current(article) ? 'text-blue' : ''" class="block duration-300 group hover:text-blue px-1 text-sm hover:translate-y-[-.1rem] w-full lg:w-1/4" :to="$i18nRoute({ name: 'NewsDetail', params: { id: article.id } })">
